...
int employee_compareByName(void* emp1, void* emp2){
    ...
    char nombre1[TAM_NOM];
    char nombre2[TAM_NOM];
...
int laQueMapea(void* emp){
    ...
    if(emp != NULL){
        ...
        int horasTrabajadas;
        employee_getHorasTrabajadas(e1, &horasTrabajadas);
        employee_setSueldo(e1, horasTrabajadas*300);
        retorno= 0;
    }
    return retorno;
}

int laQueFiltra(void* emp){
    int retorno = -1;
    Employee* e1 = (Employee*) emp;
    char empleo[TAM_NOM];
    int edad;

    employee_getEmpleo(e1, empleo);
    employee_getEdad(e1, &edad);

    if(stricmp(empleo, "programador")== 0 && edad > 20){
        retorno= 0;
    }
